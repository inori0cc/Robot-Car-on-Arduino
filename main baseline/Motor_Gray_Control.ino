...
void road_one()
{
  Car_Move(Forward,130,180);delay(2200);
  Car_Move(Forward,95,135);delay(1900);
  Car_Move(Stop,Car_speed_stop,Car_speed_stop);delay(500);
  Car_Move(Forward,25,65);delay(600);
  Car_Move(Stop,Car_speed_stop,Car_speed_stop);delay(500);
  Car_move_state_delaytime(Tracking_automatic,6000);
}

void road_two()
{
  delay(1000);
  Car_Move(Forward,135,175);delay(2400);
  Car_Move(Stop,Car_speed_stop,Car_speed_stop);delay(500);//可以写成第二次能上坡防止第一次没上去
  Car_Move(Forward,40,80);delay(600);
  Car_Move(Stop,Car_speed_stop,Car_speed_stop);delay(500);
  Car_Move(Forward,40,80);delay(600);
  Car_Move(Stop,Car_speed_stop,Car_speed_stop);delay(500);
  Car_Move(Forward,40,80);delay(600);
  ...
  Car_Move(Stop,Car_speed_stop,Car_speed_stop);delay(500);
  Car_Move(Forward,40,80);delay(600);
  Car_move_state_delaytime(Tracking_automatic,9500);
}

void road_three()
{
    Tracking_Automatic_Tracking(240);
    Car_Move(Stop,Car_speed_stop,Car_speed_stop);delay(500);
    return_color_card(); //小车走到色卡开始识别色卡
    
    Tracking_Automatic_Tracking(700);Car_Move(Stop,Car_speed_stop,Car_speed_stop);delay(500);
    return_color_ballon();
    if(color_detection_card == color_detection_ballon){
      Tracking_Automatic_Tracking(900);Car_Move(Stop,Car_speed_stop,Car_speed_stop);delay(500);
      Qingdao();
      finish = false;
      finish_all = false;
    }//小车走到第一个气球区域，并判断气球颜色。
    ...
    if(finish_all){
        Tracking_Automatic_Tracking(1300);Car_Move(Stop,Car_speed_stop,Car_speed_stop);delay(500);
        return_color_ballon();
    ...
    Car_Move(Forward, 255, 255);delay(4000);
}
